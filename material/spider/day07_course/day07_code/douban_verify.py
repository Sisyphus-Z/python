...
driver.find_element_by_xpath('//*[@id="password"]').send_keys('1111')
driver.find_element_by_xpath('/html/body/div[1]/div[2]/div[1]/div[5]/a').click()

time.sleep(1)
# 4、切换到新的iframe子页面 - 滑块验证
node = driver.find_element_by_xpath('//div[@id="TCaptcha"]/iframe')
...
# move_to_element_with_offset: 移动到距离某个节点多少像素的位置
ActionChains(driver).move_to_element_with_offset(click_node, xoffset=180, yoffset=0).perform()

# 6、使用加速度函数移动剩下的距离
# tracks: [第一个0.3秒位移, 第二个0.3...., ...]
tracks = get_tracks(28)
for track in tracks:
    # move_by_offset: 移动到距离当前位置多少的距离
